...
export default function RedBullSection() {
  ...
  return (
    <section id="redbull" className="py-24 relative overflow-hidden bg-[#0a0a0e] border-y border-red-950/60">
      {/* Background Neon Lasers */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-red-600/10 rounded-full blur-[150px] pointer-events-none"></div>
      ...
      <div className="container-custom relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div>
            ...
            <h2 className="font-heading text-5xl sm:text-6xl md:text-7xl text-white tracking-wide">
              REDBULL <span className="text-[#ff0033]">BATALLA</span> & GALLOS
            </h2>
            <p className="text-zinc-400 text-sm sm:text-base max-w-xl font-light mt-1">
              Consagración en el circuito de mayor prestigio mundial. Agilidad mental, punchlines quirúrgicos y un minuto de oro demoledor.
            </p>
          </div>

          <div className="flex items-center gap-3 bg-red-950/40 border border-red-700/50 p-3.5 rounded-2xl">
            <div className="w-12 h-12 rounded-xl bg-[#ff0033] flex items-center justify-center text-white shadow-lg shadow-red-600/50">
              <Trophy size={24} />
            </div>
            <div>
              <div className="text-[11px] font-tech text-red-300 font-bold uppercase tracking-wider">
                PALMARÉS OFICIAL
              </div>
        ...
        <div className="relative rounded-3xl overflow-hidden border border-red-500/40 bg-zinc-950 mb-16 shadow-[0_0_50px_rgba(255,0,51,0.2)] group">
          <div className="grid grid-cols-1 lg:grid-cols-12 items-center">
            ...
            <div className="lg:col-span-5 relative aspect-[4/3] lg:aspect-auto lg:h-[450px] overflow-hidden">
              <img
                ...
                alt="COREBACK Red Bull Batalla Live"
              ...
              {/* Badge on Photo */}
              <div className="absolute top-4 left-4 bg-black/80 backdrop-blur-md px-3 py-1.5 rounded-full border border-red-500/50 flex items-center gap-2">
            ...
            <div className="lg:col-span-7 p-6 sm:p-10 space-y-6">
              <div className="space-y-2">
                <span className="tag-badge">
                  <Flame size={13} className="text-[#ff0033]" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {artistData.redBullMilestones.slice(0, 4).map((item, idx) => (
                  <div
                    key={idx}
                    className="p-3.5 rounded-xl bg-zinc-900/80 border border-zinc-800 hover:border-red-500/50 transition-colors space-y-1"
                  >
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-heading text-base text-[#ff0033]">{item.year}</span>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="font-heading text-3xl sm:text-4xl text-white tracking-wide">
              PUNCHLINES <span className="text-[#ff0033]">MEMORABLES</span>
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {punchlines.map((bar, index) => (
              <div
                key={index}
                className="glass-card p-6 relative flex flex-col justify-between group hover:border-[#ff0033] transition-all"
              >
                {/* Red laser corner accent */}
                <div className="absolute top-0 right-0 w-8 h-8 overflow-hidden">
                  <div className="absolute transform rotate-45 bg-[#ff0033] text-[8px] font-tech font-bold text-white text-center w-12 top-1.5 right-[-14px]">
                    MVP
                  </div>
                </div>

                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-xs font-tech text-[#ff0033] font-bold uppercase tracking-wider">
                    <Zap size={14} />
                    <span>{bar.round}</span>
                  </div>
                ...
                <div className="pt-6 mt-4 border-t border-white/10 flex items-center justify-between text-xs font-tech text-zinc-400">
                  <span>{bar.opp}</span>
                  <span className="text-red-400 font-bold">{bar.hype}</span>
                </div>
              </div>
            ))}
          </div>
